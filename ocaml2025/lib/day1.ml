...
let score state_score = state_score.score + if state_score.state = 0 then 1 else 0 

let final_rotation amount direction state =
    amount mod 100
    |> (fun amount_mod_100 -> state + if direction then amount_mod_100 else -1 * amount_mod_100)
    |> (fun score_normalized -> state != 0 && (score_normalized <= 0 || score_normalized > 99))

let score2 state_score amount direction = state_score.score + amount / 100 + if final_rotation amount direction state_score.state then 1 else 0
