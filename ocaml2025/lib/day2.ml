...
let range until =
  let rec aux i acc = if i > until then acc else aux (i + 1) (i :: acc) in
  aux 1 []

let invalid n partition_size =
  let digits = Arith.digits_of_int n in
...
let rec range_score from until invalid_func =
  if from > until then 0
    ...
    (if invalid_func from then from else 0)
    + range_score (from + 1) until invalid_func

let sum_invalids input part =
  let invalid_func =
    if part = 1 then fun n ->
  ...
  List.map
    (fun range ->
      match range with
      | [ first; second ] -> range_score first second invalid_func
      | _ -> 0)
    ranges
